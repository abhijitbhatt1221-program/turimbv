...
(function(){
  ...
  function card(type,item){
    ...
    if(type==='package') return `<article class="package-card lift-card package-click-card" tabindex="0" role="link" aria-label="View details for ${esc(item.title)}" data-package-id="${esc(item.id)}" data-detail-url="package-detail.html?id=${esc(item.id)}"><div class="card-img" ${bgStyle(item)}>${cardImage(item,item.title)}<span class="badge orange">${esc(item.category)}</span><div class="card-img-title"><b>${esc(item.title)}</b><span>${esc(item.destination || 'TOURIM package')}</span></div></div><div class="card-body"><h3>${esc(item.title)}</h3><p>${esc(item.short)}</p><div class="meta"><span>${esc(item.duration)}</span><div style="display:flex; flex-direction:column; align-items:flex-end; line-height:1.2;">${(item.id.charCodeAt(0)+item.id.charCodeAt(item.id.length-1))%2===0 ? `<span style="text-decoration:line-through; font-size:11px; color:#94a3b8; font-weight:normal;">${money(Math.round(item.price * (1 + (((item.id.charCodeAt(1)*7)%16)+5) / 100)))}</span>` : ""}<b>${money(item.price)}</b></div></div><div class="action-row"><a class="btn teal" href="package-detail.html?id=${esc(item.id)}">View Details</a><a class="btn ghost" href="contact.html?package=${encodeURIComponent(item.title)}">Enquire</a></div></div></article>`;
